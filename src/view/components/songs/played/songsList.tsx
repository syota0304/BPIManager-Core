...
> {
  constructor(props: P & RouteComponentProps) {
    ...
    this.state = defaultState_songsList(
      initialBPIRange,
      this.props.defToday || false
    );
  ...
  sortedData = (): scoreData[] => {
    const { scoreData, orderMode, orderTitle, allSongsData, showLatestOnly } =
      this.state;
    let s = scoreData;
    let res = scoreData.sort((a, b) => {
      ...
      switch (orderTitle) {
        ...
        case 2:
          if (Number.isNaN(a.currentBPI)) return orderMode === 0 ? 1 : -1;
          if (Number.isNaN(b.currentBPI)) return orderMode === 0 ? -1 : 1;
          return a.currentBPI - b.currentBPI;
        case 3:
          const isUndefinedPlayState = 7;
          const convertPlayState = (st: number) =>
            st === isUndefinedPlayState || isNaN(st) ? -1 : st;
          return (
            convertPlayState(a.clearState) - convertPlayState(b.clearState)
          );
        case 4:
          const am = !a.missCount
              ? Infinity
              : Number.isNaN(a.missCount)
              ? Infinity
              : a.missCount,
            bm = !b.missCount
              ? Infinity
              : Number.isNaN(b.missCount)
              ? Infinity
              : b.missCount;
          return am - bm;
        case 5:
        ...
        case 6:
          return (
            (a.exScore / aFull["notes"]) * 2 - (b.exScore / bFull["notes"]) * 2
          );
        case 7:
        ...
        case 9:
          ...
          if (/-/.test(aBpm))
            aBpm =
              orderTitle === isMaxBPM ? aBpm.split("-")[1] : aBpm.split("-")[0];
          if (/-/.test(bBpm))
            bBpm =
              orderTitle === isMaxBPM ? bBpm.split("-")[1] : bBpm.split("-")[0];
    ...
    if (_showLatestSongs() && orderTitle === 2) {
      s = scoreData
        .filter((item) => item.currentBPI === Infinity)
        .sort((a, b) => {
          const aFull = allSongsData.get(genTitle(a.title, a.difficulty));
          const bFull = allSongsData.get(genTitle(b.title, b.difficulty));
          if (!aFull || !bFull) return -1;
          return (
            (b.exScore / bFull["notes"]) * 2 - (a.exScore / aFull["notes"]) * 2
          );
        });
      if (showLatestOnly) {
        return s;
  ...
  applyFilter = (state: {
    ...
    newState.showLatestOnly =
      typeof state.showLatestOnly !== "boolean" ? false : state.showLatestOnly;
    return this.setState({
      scoreData: this.songFilter(newState),
      bpm: state.bpm,
      ...
      memo: newState.memo,
      showLatestOnly: newState.showLatestOnly,
      page: 0,
    });
  };
  ...
  toggleTimeRangeDialog = () =>
    this.setState({ timeRangeOpen: !this.state.timeRangeOpen });
  ...
  openCaptureScr = () =>
    this.setState({ openCaptureScr: !this.state.openCaptureScr });
  ...
  render() {
    ...
    return (
      <Container fixed className="commonLayout" id="songsVil">
              ...
              >
                {ranges.map((item) => (
                  <MenuItem value={item.val} key={item.val}>
                    <FormattedMessage id={`Songs.Range${item.val}`} />
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
        ...
        >
          <Grid item xs={6}>
            <FormControl style={{ width: "100%" }}>
              <InputLabel>
                <FormattedMessage id="Songs.mode" />
              </InputLabel>
              <Select value={mode} onChange={this.handleModeChange}>
                {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((item) => (
                  <MenuItem key={item} value={item}>
                    <FormattedMessage id={`Songs.mode${item}`} />
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
        ...
        {timeRangeOpen && (
          <TimeRangeDialog
            handleToggle={this.toggleTimeRangeDialog}
